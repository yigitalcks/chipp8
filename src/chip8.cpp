...
bool Chip8::loadROM(const std::filesystem::path& path) {
	...
	file.read(reinterpret_cast<char *>(&m_memory[PROGRAM_ADDRESS]), static_cast<std::streamsize>(size));
	return true;
}
...
void Chip8::I_1NNN(uint16_t nnn) { 
	m_pc = nnn; 
}
...
void Chip8::I_6XNN(uint8_t x, uint8_t nn) { 
	V[x] = nn; 
}

void Chip8::I_7XNN(uint8_t x, uint8_t nn) { 
	V[x] += nn; 
}

void Chip8::I_8XY0(uint8_t x, uint8_t y) { 
	V[x] = V[y]; 
}

void Chip8::I_8XY1(uint8_t x, uint8_t y) { 
	V[x] |= V[y]; 
}

void Chip8::I_8XY2(uint8_t x, uint8_t y) { 
	V[x] &= V[y]; 
}

void Chip8::I_8XY3(uint8_t x, uint8_t y) { 
	V[x] ^= V[y]; 
}
